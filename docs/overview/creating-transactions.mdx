...
const sendTransaction = async () => {
  ...
  const destinationTxUrl = provider.getTxUrl(targetChain, destinationTx.hash)
...
For information about the flow of the components that appear through the checkout process, see [`@rarimo/react-nft-checkout`](../sdk/react-nft-checkout).
