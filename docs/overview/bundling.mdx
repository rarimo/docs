...
Bundling creates temporary addresses that Rarimo uses to store and lock tokens.
Then, Rarimo creates a temporary contract on the destination chain to unlock equivalent tokens and use them to buy NFTs.

You don't usually need to access these temporary addresses and contracts, but if you want to, you can set the bundle salt for a transaction.
...
Then you can use this salt in Rarimo transactions, which helps you determine the addresses.
