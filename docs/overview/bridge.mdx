...
The solution is available as a ready-to-use [React widget](/reference/sdk/packages-checkout#@rarimo/react-nft-checkout) that requires little effort for integration. Alternatively, a custom integration can be built using the [JS SDK](/reference/sdk/packages-checkout#@rarimo/nft-checkout).
