...
The main flow of NFT Settlement Usecase is buying NFT for tokens which will be exchanged for the needed one through one
transaction, which happens in 7 steps:
...
7. The receiver gets bought NFT in his wallet
...
it. As salt you should specify random 32 bytes length array encoded into the hex string, there is snipped how to
generate it:
```javascript
...
#### Bundle Data
To create a transaction bundle needed to specify Marketplace contract call and Web3.js and ethers packages must be
installed. We will use web3.eth.abi.encodeParameters function to encode all needed parameters into the bytes array.
