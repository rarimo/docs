...
- Moving the converted tokens to the target blockchain, which is the chain that the seller wants to accept payment on
...
1. The bridge contract calls the relayer, which receives the witness and provides it to the Rarimo contract on the target chain.
1. The Rarimo contract on the target chain unlocks the equivalent tokens on the target chain and sends them to the seller's wallet on that chain.
1. The Rarimo contract sends the purchased NFT to the customer's wallet.
