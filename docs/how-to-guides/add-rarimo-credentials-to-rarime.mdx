...
async function createConnector() {
    const snap = await enableSnap(); // connects to the snap or isntalls it
...
:::tip
Keep the UX transparent: only show the prompt only after the user explicitly clicks on **Add credential** button or a similar UI element.
:::
