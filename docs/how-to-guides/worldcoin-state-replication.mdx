...
Before getting started, please get in touch with us in [discord](https://discordapp.com/invite/rarimoprotocol) to ensure that the issuer is added to the allowlist and that a lightweight identity manager contract is already deployed to the destination chain;
