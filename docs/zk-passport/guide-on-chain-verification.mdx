...
3. Pass the `pubSignals` as calldata to the `execute()` method in the smart contract along with the `proof`.
   ...
   contract MyPassportContract is AQueryProofExecutor {
    ...
    // Builds the public signals for verification
    function _buildPublicSignals(bytes32, uint256 currentDate_, bytes memory userPayload_)
        public override returns (uint256 dataPointer_) { /* ... */ }

    ```

    Here, your contract inherits from `AQueryProofExecutor`, which handles the ZK proof validation internally.

### B. Manually Assemble the Public Signals Before Passing Them to the `verifyProof` Method
...
pragma solidity ^0.8.28;
   ...
   contract MyPassportContract is AQueryProofExecutor {
    ...
    function getEventData() public view returns (uint256) {
        // Implementation depends on your application
        return uint256(uint248(uint256(keccak256(abi.encodePacked(accessStartTimestamp)))));
    }
}
```
