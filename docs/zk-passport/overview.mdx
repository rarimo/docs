...
1. **Set Up**: Install the Rarimo App on your phone. Use the app to scan your passport with the phone's camera and NFC chip during a one-time setup.
1. **Proof Request**: When a DApp requests passport proof, scan the QR code provided by the DApp using the Rarimo App and approve the proof generation request.
...
To balance decentralization and compatibility, the Rarimo App defaults to the full verification mode and switches to light verification when necessary. This is done automatically and seamlessly for the user.
