...
1. Open the demo page <OutLink href="https://nft-checkout.rarimo.com/">NFT Checkout demo</OutLink>
   ...
   <Image className="idealImage" img={require('/img/token-agnostic-page.png')} alt="Token-agnostic page" shouldAutoDownload={() => true} />

2. Click "Purchase with any token" and connect your Metamask

   <Image className="idealImage" img={require('/img/token-agnostic-walletconnection.png')} alt="Token-agnostic wallet" shouldAutoDownload={() => true} />

3. Select the token you want to pay with(in this case, it's UNI)

   <Image className="idealImage" img={require('/img/token-agnostic-tokens.png')} alt="Token-agnostic tokens" shouldAutoDownload={() => true} />

4. Check out the estimated fees and proceed to the checkout

   <Image className="idealImage" img={require('/img/token-agnostic-receipt.png')} alt="Token-agnostic reciept" shouldAutoDownload={() => true} />

5. Approve the token spending for the Rarimo swap contract

   <Image className="idealImage" img={require('/img/token-agnostic-swap.png')} alt="Token-agnostic swap" shouldAutoDownload={() => true} />

6. Confirm the swap of UNI to WETH

   <Image className="idealImage" img={require('/img/token-agnostic-swap-confirm.png')} alt="Token-agnostic swap confirm" shouldAutoDownload={() => true} />
...
7. …and, that's it! Rarimo Relayer will pick up the transaction and automatically complete the purchase of the NFT in the destination network
   ...
   <Image className="idealImage" img={require('/img/token-agnostic-confirmation.png')} alt="Token-agnostic confirmation" shouldAutoDownload={() => true} />
...
- [Customizable react component](/ecosystem/token-agnostic-payments/guide-creating-nft-checkout-transactions#react-applications)
  ...
  <Image className="idealImage" img={require('/img/token-agnostic-button.png')} alt="Token-agnostic button for buying" shouldAutoDownload={() => true} />
