...
<p><Image img={require('/img/iden3-state.png')} alt="Iden3 State" /></p>
...
<p><Image img={require('/img/polygon-id-integration.png')} alt="Polygon ID Integration Architecture" /></p>
...
<p><Image img={require('/img/polygon-id-state-replication.png')} alt="Polygon ID State Replication Flow" /></p>

The user then generates a ZKP using the Polygon ID Wallet app and submits it to the verifier contract using Metamask.

<p><Image img={require('/img/polygon-id-zkp-flow.png')} alt="Polygon ID ZKP Generation Flow" /></p>
