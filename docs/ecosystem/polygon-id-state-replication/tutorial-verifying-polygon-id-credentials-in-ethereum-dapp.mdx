...
Let's go through the core business logic. For a start, we'll write a function, that creates the proof request in JSON format.
It will be wrapped in React's `QRCode` component that displays a QR code.
...
Service will store the proof in with a unique `verification_id` mapping.
The front-end can call the service' endpoint to get the proof information (`getJWZ` function).
