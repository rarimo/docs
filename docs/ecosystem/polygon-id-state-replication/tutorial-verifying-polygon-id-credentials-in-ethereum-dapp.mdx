...
1. The front end creates the QR code that contains the proof request and shows it on the page for the user.
  <Image className="idealImage" img={require('/img/polygon-id-integration-qr-code.png')} alt="Proof request QR-code" shouldAutoDownload={() => true} />

1. The user can scan this QR code with his Polygon ID Wallet and generate proof that he possesses valid credentials.
  <Image className="idealImage" img={require('/img/polygon-id-integration-generated.png')} alt="Proof generated, connect wallet to proceed" shouldAutoDownload={() => true} />

1. After that, the relayer will be called to replicate the state from Polygon to Ethereum.
  <Image className="idealImage" img={require('/img/polygon-id-integration-relay.png')} alt="Performing state transition" shouldAutoDownload={() => true} />

1. The user can submit the proof with his MetaMask and mint the SBT.
  <Image className="idealImage" img={require('/img/polygon-id-integration-sbt.jpg')} alt="Verified and received an SBT" shouldAutoDownload={() => true} />
