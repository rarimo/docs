...
Modified Iden3 contract, that verifies ZKP, checks that the identity hasn't been registered before and that it is unique.
...
- `REGISTER_PROOF_QUERY_ID` — unique query identifier. Used as public parameter in ZKP;
- `_issuingAuthorityWhitelist` — whitelist of issuing authorities;
- `_issuingAuthorityBlacklist` — blacklist of issuing authorities;
...
- `_onlyRegistrationContract()` — private function, that implements the logic of `onlyRegistrationContract()` modifier;
...
- `register()` — registers a user, verifying his identity and registration proof. Checks that the user is unique and hasn't been previously registered (through `RegisterVerifier` call);
