---
sidebar_label: PolygonID x Rarimo Contracts
---

# PolygonID x Rarimo Reference

This page contains technical resources related to the PolygonID x Rarimo contracts.

##  Smart Contracts

PolygonID x Rarimo integration contracts are slightly different from our other [identity contracts](/reference/contracts/identity) in order to be compatible with PolygonID Wallet, but the logic remains same.  
...
The SBT awarded to users who complete the PolygonID x Rarimo KYC verification.
...
Rarimo uses Iden3 validator and verifier contracts.
Validators perform some necessary checks and call the required verifier contract, which in turn verifies the proof.
Verifiers are generated from circom schemes.
They store the verification key and `verifyProof()` function.
`ZKPVerifier` operates on top of them, by storing the requests, their IDs and other information.
