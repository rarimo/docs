...
In the Iden3 protocol, Identity State consists of multiple Sparse Merkle Trees(SMT) that describe the status of claims associated with an identity. The state is stored on-chain and is used during the ZKP verification.
