...
```json
{
  "chain": "Name of the chain into which it is relayed according to the service configuration",
...
- `"waitConfirm": true` indicates, whether request should wait until the transaction is included in the block (`false` by default).

The response codes:
- `200` - successful relay, body contains tx hash;
- `404` - state is not transferred yet, try to repeat the request later;
