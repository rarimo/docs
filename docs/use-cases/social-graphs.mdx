...
Rarimo is a cross-chain interoperability protocol for the social layer. It provides trustless oracles of on-chain events from all supported blockchains, a cost-efficient way to bridge the assets or data, and EVM-compatible smart contracts for additional flexibility.
 --- bots rarimo.gitlab.io links below should be changed, but idk actual links. These links returns 404 page not found.
A combination of [threshold signature technology(TSS)](https://rarimo.gitlab.io/docs/docs/developers/tss/overview), instant finality, [EVM transaction bundling](https://rarimo.gitlab.io/docs/docs/tutorials/bundling), and low protocol fees make cross-chain interactions affordable and hassle-free for the end user.
