...
1. Get a KYC Age credential by scanning a QR-code [here](https://admin-panel.polygon.rarimo.com/credentials/scan-link/4f9876ca-1829-4572-9da2-1d11d986688f) with your PolygonID Wallet. Than wait 2 minutes for the identity provider state to be committed and propagated on-chain;
1. Open the [Demo DApp](https://polygon.rarimo.com/) and scan the QR-code with your PolygonID Wallet. Confirm the proof generation.
1. Once the DApp receives the proof submit it to the Verifier Contract using MetaMask;
...
If you run an identity issuer for PolygonID and want to enable the usage of ZKPs on Ethereum or another EVM-compatible network, contact us at info@rarifylabs.com.

The integration will require the following steps:
- Set up the Oracle to watch your State Contract on Polygon;
- Deploying a lightweight State Contract and a Verifier to the target chain;
- Set up a Relayer to work with your contracts;
