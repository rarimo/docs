...
1. Call the deposit method on the bridge contract. For ERC721 [`depositERC721`](../reference/contracts/interface/evm-reference#depositerc721), for ERC1155 [`depositERC1155`](../reference/contracts/interface/evm-reference#depositerc1155). Note that you could specify some code that should be called on destination chain using [bundling feature](../overview/bundling);
...
1. Call the [`depositSBT`](../reference/contracts/interface/evm-reference#depositsbt) method on the bridge contract. It checks the SBT ownership and triggers the corresponding event in the Rarimo Core;
