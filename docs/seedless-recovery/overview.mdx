...
This recovery path includes a one-year timelock, enhancing security (see below).
...
Without the **H(IMG)** value, attackers face brute-forcing a 180-bit key, an economically impractical task. Additionally, the one-year timelock provides extra protection against such attacks.
