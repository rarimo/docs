...
In this tutorial we are going to build the DApp, that verifies the age of the user and mints and SBT after that.
Given an issuer whose state exists on Polygon, we can replicate that state to another chain, e.g. Ethereum, so that the claims he issued can be proven in Ethereum Dapp.
...
- `VerifiedSBT` contract, that is the implementation of Souldbound token;
- `QueryVerifier` contract, that will verify the proof and call the SBT contract to mint the token on successful verification;
...
Our goal, as stated before - to mint an SBT on successful age verification.
...
contract VerifiedSBT is ERC721Enumerable {
    ...
    uint256 public nextTokenId; // nonce, that should be increment with every new minted SBT
    string public tokensURI; // token URI as in NFTs

    // may be leaved as it, or u can additionally ask for a tokensURI and set it in the constructor
    constructor() ERC721("Name", "Symbol") {}

    // modifier, that makes mint function callable only by the verifier contract
...
Provided functions are empty, so we need to fill them:
- `onlyVerifier()` – checks, whether the sender is the same as `verifier` variable and proceeds in such a case;
- `getNextTokenId()` – returns the next token id;
- `setVerifier(...)`, `setTokensURI(...)` – set the corresponding value to the provided one;
- `mint(...)` – mints the token to the provided address, with next token id (hint: call the ERC721 parent function `_mint`);
...
Great! Now we need to write a `QueryVerifier` and we are almost ready to use Polygon ID State Replication.
As said before, this contract will verify the proof and call `VerifiedSBT` contract to mint the SBT for the user.
...
We need both mappings, so one address would be able to pass the verification only once and one identity can be verified only by one address.

We will also need the interface for `VerifiedSBT` contract, that contains all the functions we created:
...
We don't need to implement the main `verify(...)` function, because our contract inherits iden3 `ZKPVerifier`, that already has it and does all the job.
...
contract QueryVerifier is ZKPVerifier {
    ...
    // returns boolean, whether the identity is verified
...
- `setSBTContract(...)` – sets the destination of SBT contract;
...
You can provide other information as well, for background, see [Polygon ID Wallet-SDK Docs](https://devs.polygonid.com/docs/wallet/wallet-sdk/polygonid-sdk/iden3comm/auth-requests/).
...
After getting a JWZ from `getJWZ(...)` function, we can submit our proof to the on-chain contract, but it may not be accepted if the state wasn't transited.
To be sure, that the state exists on destination chain, we should call the relayer.

The following code can be used for that:
...
Finally, after transiting the state and getting the JWZ we can submit the proof, so it can be verified:
