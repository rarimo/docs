...
For a complete example, see [SDK](../reference/sdk).
...
1. Connect to a wallet and create a transaction operation with the [`createCheckoutOperation()`](https://rarimo.github.io/js-sdk/functions/_rarimo_nft_checkout.createCheckoutOperation.html) function:
   ...
   ```ts
   import { createCheckoutOperation, EVMOperation, CheckoutOperationParams } from '@rarimo/nft-checkout'
...
This step assumes that you have set the target transaction and chain for the operation as described in [SDK](../reference/sdk):
...
1. Call the operation's [`checkout()`](https://rarimo.github.io/js-sdk/interfaces/_rarimo_nft_checkout.CheckoutOperation.html#checkout) method to run the transaction:
