...
Many preparations were done, and now is the time to eat a cherry on the cake: prove that you’ve indeed received data from certain neural network. All the magic is done by running one command:
...
Under the hood it produces Groth16 proof for given `.wtns` and `.zkey` files. In the output it will give you  two json’s `public.json`  and `proof.json` . First one contains all the information about public circuit outputs(address of the contract, nonce, threshold, output of the neural network, etc).
...
As we previously mentioned *Bionetta*🌿 produces verification keys and Solidity verification contract. In case if are not familiar with Solidity you can run one command to verify your proof:
