...
The following [guide](/products/rarimo-protocol/validator-node-operator-guide) explains how to set up a validator node and how to stake the tokens.
...
`Tokens per Share = validator.Tokens() / validator.TotalShares() `Shares` are an accounting mechanism, not a fungible asset.
