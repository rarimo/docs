...
<!-- comment: maybe it is worth to add a link with the statement "Addresses of other contracts, such as LightweightState, can be found <here> -->
...
After scanning this QR code, the user's wallet will send the response (proof in JWZ format) using the `callbackUrl`.
