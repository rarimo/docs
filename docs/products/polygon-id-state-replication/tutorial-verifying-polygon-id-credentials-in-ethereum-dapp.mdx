---
title: Polygon ID State Replication Usage
---
...
1. The front-end creates the QR code, that contains the proof request and shows it on the page, so the user can scan it.
...
1. User can submit the proof with his MetaMask and mint the SBT.
...
If you want, you can deploy currently created contracts by yourself for testing.
...
<!-- comment: maybe it is worth, to add a link with a statement "Addresses of other contracts, such as LightweightState, can be found <here> -->


But, if you want to use this integration for production purposes, we recommend you to clone from our [GitHub](https://github.com/rarimo/polygonid-integration-contracts/tree/master) and extend contracts with your business logic.
...
<!-- comment: I mention this link at the end of the document, maybe remove here? -->
...
It will be wrapped in React's `QRCode` component, and so, displayed as a QR code.
After scanning this QR code, user's wallet will send the response (proof in JWZ format) using the `callbackUrl`.
...
If you want to tweak or expand this example, we recommend you to clone from the GitHub – [front-end](https://github.com/rarimo/web-client-polygonid/tree/main) and [smart contracts](https://github.com/rarimo/polygonid-integration-contracts), make the changes you need and use it for your purposes.