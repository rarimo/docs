...
<Image className="idealImage" img={require('/img/bridging-flow.png')} alt="Bridging Flow" shouldAutoDownload={() => true} />
...
Rarimo has integrated the <OutLink href="https://www.circle.com/en/cross-chain-transfer-protocol">Circle CCTP protocol</OutLink> to move USDC tokens between the chains without needing liquidity pools.
...
<Image  className="idealImage" img={require('/img/token-agnostic-receipt.png')} alt="Token Agnostic Payments Example" shouldAutoDownload={() => true} />
...
The solution is available as a ready-to-use [React widget](/ecosystem/token-agnostic-payments/js-sdk-reference#rarimoreact-nft-checkout) that requires little effort for integration. Alternatively, a custom integration can be built using the [JS SDK](/ecosystem/token-agnostic-payments/js-sdk-reference#rarimonft-checkout).
