# Rarimo x WorldCoin: Identity State Replication
...
![State Replication flow](/img/zkp.png)
...
1. It must be protected from spoofing of the state;
...
- `WorldIDIdentityManager` has to be updated any time support for a new destination chain is added;
...
![On-Demand Identity State Replication](/img/polygon-eth-replication.png)
...
## Integrating World ID with Rarimo State Replication

The integration will consist of creating a Rarimo oracle for tracking the state updates on Ethereum and creating a lightweight version of a World ID smart contract for the target chains that accepts updates with Rarimo's TSS and allows lazy updates.
No additional changes to World Coin's protocol are required.