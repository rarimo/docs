---
title: World ID State Replication Usage
---
...
In this tutorial, we will build the DApp, that will ask the user to provide a proof of personhood, using the World App.
So, having the Worldcoin issuer and state contract anchored on Ethereum, we can replicate its state to another chain, e.g. Polygon, so that the proofs can be used in Polygon DApp.

## User flow
1. Firstly, user will be asked to connect his Metamask wallet.
    After connecting the wallet, user can go through the verification process.

    ![Verify with World ID](/img/world-id-verify.png)

1. After clicking on the button, QR code is shown, that user can scan with his World App and generate proof that he is a real human.

    ![QR Code, that can be scanned with World App](/img/world-id-qr-code.png)

1. After successful proof generation, the relayer will be called to replicate the state from Ethereum to Polygon.
    
    ![Proof Generated, transiting the state, using the relayer](/img/world-id-proof-generated.png)

1. After proof is submitted, user can review the correctness, using the transaction hash
...
Let's start by writing a verifier contract, that will verify user's proof of personhood.
...
Proof will be verified by another contract – [SemaphoreVerifier](https://github.com/worldcoin/world-id-contracts/blob/main/src/SemaphoreVerifier.sol), that is fully forked from World ID and we don't need to change it.
Verifier also uses [Initializable from OpenZeppelin](https://github.com/OpenZeppelin/openzeppelin-upgrades/blob/master/packages/core/contracts/Initializable.sol), remember to clone them.
...
contract Verifier is IVerifier, Initializable {
    ...
    address public identityManager; // address of the identity manager contract, deployed by Rarimo team on destination chain
...
As previous hook, can contain any logic you want.
...
We have not filled the hooks, their logic is up to you.
...
You can deploy recently created `Verifier` contract, together with `SemaphoreVerifier` by yourself.
...
1. Fill in the `deploy/config/config.json` file.
    ...
    ```
    where `signer` is the address of Rarimo TSS signer, `sourceStateContract` is the address of WorldID manager contract, and `chainName` is the name of the network (e.g. Ethereum, Sepolia, Mumbai), where the contract are being deployed to.
...
We use World ID Kit, that provides rich and easy to use functionality and handles proof processing.
Namely, we will use `IDKitWidget` component, that will create the QR code, check the correctness of the proof, etc.
...
Here is the part of landing page with the `IDKitWidget`, which you can fill and use at your page:
...
QR code contains the request to generate the proof in user's World App.
...
At this point user has passed the verification and you can be sure, that this is a real person, and not a bot and provide him the full access to your DApp.

If you want to tweak or expand this example, we recommend you to clone [front-end](https://github.com/rarimo/web-client-worldcoin-id/tree/main) and [smart contracts](https://github.com/rarimo/worldid-integration-contracts/) from the GitHub, make the changes you need and use it further.