...
description: 'Information about the smart contracts, used for passport-derived profiles.'
---
...
Set of contracts, that make possible on-chain identities issuance, that are connected to users passports, without revealing any private data.
...
1. <OutLink href="https://github.com/rarimo/passport-contracts/tree/master/contracts/authenticators">Authenticators</OutLink>
 — perform the passport Active Authentication (AA) and calls one of the verifier contracts, based on the provided passport type, to verify ZKP.
1. <OutLink href="https://github.com/rarimo/passport-contracts/tree/master/contracts/dispatchers">Dispatchers</OutLink>
 — Connect the main `Registration` contract with different authenticators, that correspond to different passport types.
...
1. <OutLink href="https://github.com/rarimo/passport-contracts/tree/master/contracts/registration">Registration</OutLink>
 — primary section with main `Registration` contract, that:
    ...
    * Allows reissuance of previously revoked identity;
    * Allows registration and revocation of ICAO members certificates;
...
- `_certificateInfos` — internal mapping from hash of certificate key to the information about the certificate;
- `_passportInfos` — internal mapping from the hash of passport public key to the information about the passport;
- `_identityInfos` — internal mapping from identity public key to the information about corresponding identity;
...
- `revokeCertificate(...)` — revokes the ICAO member certificate and emits `CertificateRevoked` event with certificate key;
...
There are also several getter functions, that return data from mappings, such as `getPassportInfo(key)` — returns passport info for provided passport key, but they are not so important, so they weren't mentioned.
...
Authenticator for `ECDSA_SHA1` passport types. Verifies ECDSA signature on passport's public key.
...
Authenticator for `RSA_SHA1` passport types. Verifies RSA signature on passport's public key, that was hashed with a random salt.
...
- `authenticate()` — checks active authenticattion of a passport via RSA signature, where message is SHA1 hash of challenge+salt;
